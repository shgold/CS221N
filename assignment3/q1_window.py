...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
    	...
    	for i in xrange(len(sentence)):
    		words = padded_sentence[i:i+1+2*window_size]
    		sample = ([v for word in words for v in word], labels[i])
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding_lookup = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder, name='embedding_lookup')
        embeddings = tf.reshape(embedding_lookup, (-1, self.config.n_window_features * self.config.embed_size))                                                                                                           
        ### END YOUR CODE
        return embeddings
