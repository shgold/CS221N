...
class Config(object):
    ...
    hidden_size = 200
    batch_size = 2048
    n_epochs = 30       # Changed from 10 to make loss smaller.
    lr = 0.001
...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        w_init = xavier_weight_init()

        W = w_init((self.config.n_features*self.config.embed_size,
                                self.config.hidden_size)) 
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))
        U = w_init((self.config.hidden_size, self.config.n_classes))
...
def main(debug= False):     # Set debug = True when do not want to run whole program.
    print 80 * "="
    print "INITIALIZING"
    print 80 * "="
    config = Config()
